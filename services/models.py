...
class Categories(models.Model):
    ...
    slug = models.SlugField(max_length=200,
                            ...
                            blank=True,
                            null=True)
    svg_image = models.FileField(upload_to='svg_images',
                              blank=True,
                              null=True, 
                              verbose_name='Фото')
...
class Products(models.Model):
    ...
    base_price = models.IntegerField(blank=True, null=True, verbose_name="Фиксированная цена")
...
class ProductsExample(models.Model):
    product = models.ForeignKey(Products,
                                ...
                                verbose_name='Продукт')
    image = models.ImageField(upload_to='product_examples/',
...
class Price(models.Model):
    product = models.ForeignKey(Products,
                                ...
                                related_name="prices",
                                verbose_name="Продукт")
    masters = models.ManyToManyField(Masters,
                               related_name="prices",
                               verbose_name="Мастер",
                               blank=True)
    hair_length = models.ForeignKey(HairLength,
                                    on_delete=models.CASCADE,
    ...
    def __str__(self):
        masters_str = ", ".join([str(master) for master in self.masters.all()])
        hair_length_str = f"({self.hair_length.name})" if self.hair_length else "Без длины"
        return f"{self.product.name} {hair_length_str} - {masters_str} - {self.min_price} ₽ - {self.max_price} ₽"
